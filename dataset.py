...
class create_data(Dataset):
    ...
    def __getitem__(self,index):
        ...
        depth = torch.nn.functional.interpolate(torch.from_numpy(depth_img).unsqueeze(0).unsqueeze(1), size=(self.config.IMAGE_MIN_DIM, self.config.IMAGE_MAX_DIM), mode="bilinear",align_corners=False).numpy()
